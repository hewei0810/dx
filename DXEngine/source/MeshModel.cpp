...
{
	CStdString strPathname = CGlobalManager::GetManager()->GetResPath() + pathname;
	CStdString strPath(strPathname);
	...
	if ( FAILED(D3DXLoadMeshFromXA(strPathname.GetData(), D3DXMESH_MANAGED, m_pD3DDev, NULL, &pBufferMaterials, NULL, &m_dwNumMaterials, &m_pMesh) ) )
		return false;
	...
	for (DWORD i = 0; i < m_dwNumMaterials; i++)
	{
		...
		CStdString strTmp = strPath + pMaterials->pTextureFilename; 
		if ( FAILED(D3DXCreateTextureFromFileA(m_pD3DDev, strTmp.GetData(), &pTex)) )
			return false;
		m_pArrTexPtr[i] = pTex;
	}
