...
CD3DRenderer* CD3DRenderer::sm_pD3DRenderer = NULL;

CD3DRenderer::CD3DRenderer(HINSTANCE hInstance, HWND hWnd)
: m_hInst(hInstance)
, m_hWnd(hWnd)
, m_pD3DDev(NULL)
, m_pInputDev(NULL)
...
{
	SAFE_DELETE(m_pD3DDev)
	SAFE_DELETE(m_pInputDev)
	SAFE_DELETE(m_pScene)
}

bool CD3DRenderer::InitInstance(HINSTANCE hInstance, HWND hWnd)
{
	if (sm_pD3DRenderer == NULL)
	{
		sm_pD3DRenderer = new CD3DRenderer(hInstance, hWnd);
		if (!sm_pD3DRenderer->CreateDevice())
			return false;
		if (!sm_pD3DRenderer->InitSetting())
			return false;
	}
...
void CD3DRenderer::ExitInstance()
{
	SAFE_DELETE(sm_pD3DRenderer)
}

bool CD3DRenderer::CreateDevice()
{
	// 获取D3D9接口
	...
	// 创建D3D设备
	if ( FAILED(pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd, vp, &m_d3dpp, &m_pD3DDev)) )
		return false;

	// 创建输入设备
	m_pInputDev = new CDInput;
	if (!m_pInputDev->CreateDevice(m_hInst, m_hWnd))
		return false;
...
bool CD3DRenderer::SetScene(CScene* pScene)
{
	SAFE_DELETE(m_pScene)

	m_pScene = pScene;
	if (!m_pScene->InitScene())
	{
		::MessageBoxA(m_hWnd, "ERROR", "初始化场景失败！", MB_OK);
		return false;
	}
...
bool CD3DRenderer::Update()
{
	...
	m_pScene->UpdateScene(fElapsedTime);

	return true;
}

bool CD3DRenderer::Render()
{
